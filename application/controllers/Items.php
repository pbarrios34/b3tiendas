	...
	*/
	...
	function save_item_pricing($item_id=-1)
	{
		...
		$item_data = array(
			...
			'previous_cost'=>$this->input->post('previous_cost'),
		...
		if($this->Item->save($item_data,$item_id))
		{
			...
			foreach($variations as $variation_id => $data)
			{
				...
				$data['previous_cost'] = $data['previous_cost'] ? $data['previous_cost'] : NULL;
	...
	function save($item_id=-1)
	{
		...
		$item_data = array(
			...
			'expiration'			=> 	$this->input->post('item_expiration') ? $this->input->post('item_expiration') : 0,
	...
	function _excel_get_header_row($include_location_columns = 0)
	{
		...
		$header_row[] = lang('common_secondary_category');
		...
		$header_row[] = lang('items_need_expiration');
	...
	{			
		...
		foreach ($data as $r) 
		{
			...
			$sub_categories = $this->Item->get_secondary_categories_full_route($r->item_id)->result();
			$sub_categoryArray = json_decode(json_encode($sub_categories), true);
			...
			$to_export[lang('common_secondary_category')][] = isset($sub_categoryArray[0]["sub_category"]) ? $sub_categoryArray[0]["sub_category"] : '';
			...
			$to_export[lang('items_need_expiration')][] = $r->expiration ? 'y' : '';
	...
	{		
		...
		$fields[] = array('Name' => lang('items_need_expiration'), 'key' => 'expiration');
	...
	function complete_excel_import()
	{
		...
		for ($i = 0; $i < $numRows -1; $i++)
		{
			...
			foreach($fields as $field)
			{
				...
				{		
					if($field['key'] == 'expiration') {
						$item_expiration = $columns_with_data[$key]['data'][$i] == 'y' ? 1 : 0;
					}	
			...
			$item_data['deleted'] = 0;
			$item_data['expiration'] = $item_expiration;
