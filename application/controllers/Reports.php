...
{	
	...
	function generate($report)
	{
		...
		{ 
			...
			if(isset($output_data['headers']['summary'])) {
				if( $output_data['title'] === 'Detalle de recibos' ){
					$creation_date_heder = [
						array(
					...
					foreach($output_data['headers']['summary'] as $key => $heder) {
						if( $heder['data'] === 'Fecha' ){
							$output_data['headers']['summary'][$key]['data'] = 'Fecha de recepción';
						}
					}
					array_splice($output_data['headers']['summary'], 3, 0, $creation_date_heder);
					foreach($output_data['summary_data'] as $keys => $col_key) {
						if( $this->Receiving->get_info( $col_key[0]['detail_id'] )->result_array(  )[0]['suspended'] == '1'){
							$output_data['summary_data'][$keys][2]['data'] = 'Suspendido';
						}
						$creation_date_data = [
							array(
								'data' => date("d-m-Y", strtotime($this->Receiving->get_info( $col_key[0]['detail_id'] )->result_array(  )[0]['rev_creation_date'])),
								'align' => 'left',
							)
						];
						array_splice($output_data['summary_data'][$keys], 3, 0, $creation_date_data);
					}
				}
	...
	public function get_previous_register_close_amount() {
		$register_id = $this->input->post('register_id');
		$register_log_id = $this->input->post('register_log_id');
		return print_r( json_encode( to_currency($this->Register->get_previous_register_close_amount( $register_id, $register_log_id ) ) ) );
	}
	...
	function export_recv($recv_id)
	{
		...
		$header_row[] = lang('common_quantity_received');
		$header_row[] = lang('common_sub_total');
		$header_row[] = lang('common_tax');
		$header_row[] = lang('reports_difference');
		...
		foreach($this->Receiving->get_receiving_items($recv_id)->result() as $item)
		{
			...
			$row[] = to_quantity($item->quantity_received);
			$row[] = to_currency_no_money($item->subtotal);
			$row[] = to_currency_no_money($item->tax);
			$row[] = to_currency_no_money($item->profit);
