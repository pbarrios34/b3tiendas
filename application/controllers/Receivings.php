...
{	
	...
	function edit_line_total($line)
	{
		...
		{
			$item->unit_price = -1*((100*$total)/($item->quantity_received*($item->discount-100)));
		}*/
	...
	function edit_item($line)
	{
		...
		if ($this->form_validation->run() != FALSE)
		{
			if($this->input->post("name"))
			{
				...
				$new_cost_price = $this->cart->get_item($line);

				if( 
					( ( $this->input->post("name") === 'quantity_received' ) && ( $new_cost_price->quantity < $$variable ) ) 
					||
					( ( $this->input->post("name") === 'quantity' ) && ( $new_cost_price->quantity_received > $$variable ) )
				){
					$data['error'] = lang('receivings_quantity_error');
					$this->_reload($data);
					return;
				}
				if( 
					$this->cart->get_mode() == 'receive' &&
					(
						( $variable === 'quantity' ) && ( $$variable < 0 ) 
						||
						( $variable === 'quantity_received' ) && ( $$variable < 0 ) 
					)
				)
				{
					$data['error'] = lang('receivings_quantity_less_0_error');
					$this->_reload($data);
					return;
				}
	...
	{	
		$this->load->model('Receiving');
		$recv_id = (int) $this->cart->receiving_id;
		$actual_location_id = $this->Employee->get_logged_in_employee_current_location_id();
		$there_is_some_without_exp_date = false;

		$there_is_some_without_exp_date = false;
		foreach( $this->cart->get_items() as $item ) {
			if ((int)$this->Receiving->get_if_have_expired($item->item_id) == 1 && is_null($item->expire_date)) {
				$there_is_some_without_exp_date = true;
				break;
			}
		}

		
		if( $this->Receiving->validate_creation_location_recv( $recv_id, $actual_location_id ) ){
			if (!$this->Employee->has_module_action_permission('receivings', 'complete_receiving', $this->Employee->get_logged_in_employee_info()->person_id))
			{
				$this->_reload(array('error' => lang('receivings_you_do_not_have_permission_to_complete_receiving')), false);
				return;				
			}	
			if ( $there_is_some_without_exp_date ){
				$this->_reload(array('error' => lang('receivings_some_without_expired_date')), false);
				return;				
			}	
			...
			$this->_reload(array('error' => lang( 'receivings_not_same_location_on_complete_recv' ) ), false);
			return;	
		}
	}
	...
	function download_receipt($receiving_id)
	{
		...
		$current_location = $this->Location->get_info($receiving_info['location_id']);
		$data['transfer_from_location'] = $current_location->name;
		$data['rev_creation_date'] = $receiving_info['rev_creation_date'];
	...
	{
		if (!$this->Employee->has_module_action_permission('receivings', 'suspend_receiving', $this->Employee->get_logged_in_employee_info()->person_id))
		{
			$this->_reload(array('error' => lang('receivings_you_do_not_have_permission_to_suspend_receiving')), false);
			return;				
		}
	...
	function unsuspend($recv_id = 0)
	{
		...
		if( !$this->Receiving->there_is_someone_editing( $receiving_id )['result'] ){
	
			$receiving_data = array(
				'some_editing' => 1,
				'person_editing' => $this->Employee->get_logged_in_employee_info()->person_id
			);
			$this->Receiving->update($receiving_data, $receiving_id);
		}
		$this->_reload(array(), false);
	}
	...
	function receipt($receiving_id)
	{
		...
		$data['transaction_time']= $receiving_info['receiving_time'];
		$emp_info=$this->Employee->get_info($receiving_info['employee_id']);
		$data['override_location_id'] = $receiving_info['location_id'];
		$data['suspended'] = $receiving_info['suspended'];
	...
	function edit($receiving_id)
	{
		...
		$this->Receiving->update($receiving_data, $this->cart->receiving_id);
	...
	{		
		...
		{
			if ($this->Employee->has_module_action_permission('receivings', 'show_can_transfer_items_locations', $this->Employee->get_logged_in_employee_info()->person_id)) 
			{
		...
		{
			if ($this->Employee->has_module_action_permission('receivings', 'show_store_account_payment', $this->Employee->get_logged_in_employee_info()->person_id)) 
			{
	...
	{
		$receiving_id = $this->cart->receiving_id;
		$this->cart->destroy();
		$this->cart->save();

		if( isset( $this->Receiving->there_is_someone_editing( $receiving_id )['person_editing'] ) && $this->Receiving->there_is_someone_editing( $receiving_id )['person_editing'] == $this->Employee->get_logged_in_employee_info()->person_id ){
	...
	private function _get_shared_data()
	{
		...
		if ($this->Employee->has_module_action_permission('receivings', 'show_receiving_mode', $this->Employee->get_logged_in_employee_info()->person_id)) 
		{
			$modes_array = array('receive'=>lang('common_receiving'));
		}
		
		if ($this->Employee->has_module_action_permission('receivings', 'show_return_items', $this->Employee->get_logged_in_employee_info()->person_id)) 
		{
			$modes_array['return']=lang('receivings_return');
		}

		if ($this->Employee->has_module_action_permission('receivings', 'show_purchase_order', $this->Employee->get_logged_in_employee_info()->person_id)) 
		{
			$modes_array['purchase_order']=lang('receivings_purchase_order');
		}

		$data['modes']=$modes_array;
	...
	function save_file_on_payments_view() {
		$this->load->model('Appfile');
		$image_file_id = $this->Appfile->save($_FILES['value']['name'], file_get_contents($_FILES["value"]['tmp_name']));
	...
	function delete_custom_field_value_with_redirect($k)
	{
		$custom_field = "custom_field_".$k."_value";
		$this->cart->$custom_field = NULL;
		$this->cart->save();
		redirect('invoices/view/supplier/910');
	}
	...
	function discount_all()
	{
		...
		elseif ($this->input->post('name') == 'discount_all_flat')
		{
			...
			$discount_amount = strpos($this->input->post('value'), '%',0) !== FALSE ? (($this->cart->get_subtotal_receivings() + $this->cart->get_discount_all_fixed()) * (float)$this->input->post('value')/100) : (float)$this->input->post('value');
	...
	function update_custom_field_invoice(  ){
		$receiving_id = (int) $this->input->post('receiving_id');
		$custom_field_data = $this->input->post('custom_field_data');
		$invoice_id = $this->input->post('invoice_id');
		$for_delete = (int) $this->input->post('for_delete');
		$custom_field_value = !$for_delete ? $this->cart->$custom_field_data : NULL;
		$receiving_data[$custom_field_data] = $custom_field_value;
		$this->cart->$custom_field_data = NULL;

		if( $this->Receiving->update( $receiving_data, $receiving_id ) ){
			$this->session->set_flashdata('success', '¡El recivo '.$receiving_id.' se actualizo correctamente!');
		}else{
			$this->session->set_flashdata('error', 'Hubo un error al querer actualizar el recivo '.$receiving_id);
		}
		redirect('invoices/view/supplier/'.$invoice_id, 'location ');
	}

	function quick_modal($id = null, $redirect_code = 0) {	
		$this->load->view('items/confirm_items_to_exp');
	}

	function save_items_quantity_expiration(  ){
		$quantity_diff = (int)$this->input->post('quantity') - (int)$this->input->post('item_quantity_expiration_name');
		$items = json_decode( $this->input->post('items') );
		$item_id = $this->input->post('item_id');
		$employee_id = $this->input->post('employee_id');
		if( $quantity_diff === 0 ){
			if( $this->Receiving->update_items_quantity_exp( $items, $employee_id ) ){
				$this->session->set_flashdata('success', '¡El producto '.$item_id.' se actualizo correctamente!');
			}else{
				$this->session->set_flashdata('error', 'Hubo un error al querer actualizar el producto '.$item_id);
			}
		}else{
			if( $this->Receiving->update_items_quantity_exp_no_equal( $items, $employee_id, $quantity_diff ) ){
				$this->session->set_flashdata('success', '¡El producto '.$item_id.' se actualizo correctamente!');
			}else{
				$this->session->set_flashdata('error', 'Hubo un error al querer actualizar el producto '.$item_id);
			}
