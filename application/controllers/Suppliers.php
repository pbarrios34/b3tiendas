			...
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				for($k = 2;$k<=$num_rows; $k++)
				{
					...
					for($a=1;$a<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$a++)
					{
						...
						{
							$supplier_data["custom_field_{$a}_value"] = $sheet->getcellByColumnAndRow(13+$a,$k);
							...
							{
								$supplier_data["custom_field_{$a}_value"] = strtotime($sheet->getcellByColumnAndRow(13+$a,$k));
							}
							else
							{
								$supplier_data["custom_field_{$a}_value"] = $sheet->getcellByColumnAndRow(13+$a,$k);
