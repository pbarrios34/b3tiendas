...
class Barcode extends MY_Controller 
{
	...
	{
		$text = H(rawurldecode($this->input->get('text')));
		$barcode = H(rawurldecode($this->input->get('barcode')));
		$scale = H($this->input->get('scale') ? $this->input->get('scale') : 1);
		$thickness = H($this->input->get('thickness') ? $this->input->get('thickness') : 30);
		if ($type == 'png')
		{
		...
		elseif($type=='svg')
		{
			$font_size = H($this->input->get('font_size') ? $this->input->get('font_size') : 13);
