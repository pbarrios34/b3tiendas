...
class Sales extends Secure_area
{
	...
	function register_add_subtract($mode,$payment_type,$return = 'sales')
	{
		$data = array();
		$data['mode'] = $mode;
		$data['payment_type'] = $this->input->post('transaction_type');
		$data['return'] = $return;
		$cash_register = $this->Register->get_current_register_log();
		...
		if ($this->input->post('amount') != '') 
		{
			...
			if ($mode == 'add')
			{
				$payment_data = array('total_payment_additions' => $additions[$this->input->post('transaction_type')]+$this->input->post('amount'));
				$message = lang('sales_cash_successfully_added_to_drawer');
			}
			else
			{
				$payment_data = array('total_payment_subtractions' => $subtractions[$this->input->post('transaction_type')]+$this->input->post('amount'));
				$message = lang('sales_cash_successfully_removed_from_drawer');
			}
			$this->Register->update_register_log_payment($cash_register->register_log_id,$this->input->post('transaction_type'),$payment_data);
			...
			$register_audit_log_data = array(
				...
				'payment_type' => $this->input->post('transaction_type'),
			...
			$last_cash_receipt = $this->Register->get_last_register_log_audit();
			
			$this->session->set_flashdata('cash_drawer_add_subtract_message', $message);
			
			if ($return == 'sales')
			{
				redirect('sales/cash_receipt/'.$last_cash_receipt);	
			}
	...
	function customer_search()
	{
		...
		if ($this->config->item('enable_customer_quick_add'))
		{
			$suggestions[] = array(
				...
				'value' => 'QUICK_ADD|'.$this->input->get('term'),
				'label' => lang('customers_add_new_customer').' '.$this->input->get('term'),
				'quick-add' => true, // Agregar un identificador especial
			);
	...
	{		
		...
		if( is_null( $this->input->post('amount_tendered') ) ){
			$amount_tendered = $this->input->post('customer_amount_tendered');
		}
		//Percent of amount due
		if(strpos($amount_tendered,'%') !== FALSE)
		{
			$percentage = (float)$amount_tendered;
			$_POST['amount_tendered'] = $this->cart->get_amount_due()*($percentage/100);
		}
		
		$data=array();
		if( is_null( $this->input->post('amount_tendered') ) ){
			$this->form_validation->set_rules('customer_amount_tendered', 'lang:sales_amount_tendered', 'required|callback_payment_check');
		}else{
			$this->form_validation->set_rules('amount_tendered', 'lang:sales_amount_tendered', 'required|callback_payment_check');
		}
		
		if(( $this->input->post('payment_type') !== lang('common_giftcard') ) && ( $this->input->post()['payment_type'] == 'Tarjeta de crédito' ) )
		{
			if (($this->form_validation->run() == FALSE))
			{				
		
				if( ( $amount_tendered === '0' || $amount_tendered === '0.00' ) && $this->cart->get_total() != 0 )
				{
		...
		if(($this->input->post('payment_type') == lang('common_ebt') && ($amount_tendered + $this->cart->get_payment_amount(lang('common_wic'))+ $this->cart->get_payment_amount(lang('common_ebt'))) > $this->cart->get_ebt_total_amount_to_charge()+1e-6) || ($this->input->post('payment_type') == lang('common_wic') && ($amount_tendered +  $this->cart->get_payment_amount(lang('common_ebt')) + $this->cart->get_payment_amount(lang('common_wic'))) > $this->cart->get_ebt_total_amount_to_charge()+1e-6))
		{
		...
		if ( $payment_type == lang('common_points') )
		{
			...
			if ($amount_tendered > to_currency_no_money($customer_info->points) || $amount_tendered <=0 || $this->cart->get_amount_due() <= 0)
			{
			...
			$payment_amount = min($max_points * $this->config->item('point_value'), to_currency_no_money($amount_tendered * $this->config->item('point_value')), $this->cart->get_amount_due());
		}
		elseif ( $payment_type == lang('common_giftcard') )
		{
			if(!$this->Giftcard->exists($this->Giftcard->get_giftcard_id($amount_tendered)) || $this->Giftcard->is_integrated($this->Giftcard->get_giftcard_id($amount_tendered)) || $this->Giftcard->is_inactive($this->Giftcard->get_giftcard_id($amount_tendered)))
			{
				$data['error']=lang('sales_giftcard_does_not_exist');
				
				if ($this->cart->get_total() < 0)
				{
					$data['prompt_to_create_giftcard'] = $amount_tendered;
				}
			...
			$payment_type=$this->input->post('payment_type').':'.$amount_tendered;
			...
			{
				$data['error']=lang('sales_giftcard_balance_is').' '.to_currency( $this->Giftcard->get_giftcard_value( $amount_tendered ) ).' !';
				$this->_reload($data);
				return;
			}
			elseif ( ( $this->Giftcard->get_giftcard_value( $amount_tendered ) - $this->cart->get_total() ) > 0 )
			{
				$data['warning']=lang('sales_giftcard_balance_is').' '.to_currency( $this->Giftcard->get_giftcard_value( $amount_tendered ) - $this->cart->get_total() ).' !';
			}
			$payment_amount=min( $this->cart->get_amount_due(), $this->Giftcard->get_giftcard_value( $amount_tendered ) );
		}
		else
		{
			$payment_amount=$amount_tendered;
		}
	...
	function complete()
	{
		...
		error_log( "FELPLEX_LOG - Time to enter controller: ".( microtime(true) - $reference_time ) );
		...
		{
			$cf=$this->Sale->get_cf_customer_default_sale()->result();
			$d_cf= json_decode(json_encode($cf), true);

			$data['customer_id'] = strval($d_cf[0]["person_id"]);
			$data['cart']->customer_id = strval($d_cf[0]["person_id"]);
		}

		for ($i=1; $i<=10; $i++)
		{
			$custom_field = $this->Sale->get_custom_field($i, 'name');
			if($custom_field == "Tipo De Documento")
			{
				$cfnumber = $i;
			}
		}

		if (!isset($cfnumber) || $cfnumber == null) {
			$cfnumber = 1;
		}

		$custom_field_n = "custom_field_".strval($cfnumber)."_value";

		if($data['cart']->$custom_field_n)
		{
			$data[$custom_field_n]= strval($data['cart']->$custom_field_n);
		}
		else if($data['cart']->$custom_field_n == null)
		{
			$data[$custom_field_n] = strval("FACT");
			$data['cart']->$custom_field_n = strval("FACT");
		}
		...
		if( 
			(
				...
				|| is_null( $this->Customer->get_info( $data['customer_id'] )->account_number ) 
				|| $this->Customer->get_info( $data['customer_id'] )->account_number == '' 
				|| $this->Customer->get_info( $data['customer_id'] )->account_number == 'CF'
			)
			&& $data['total'] >= 2500
		){
			$this->_reload(array('error' => lang('sales_alert_quantity_up_2500')), false);
			return;
		}
		...
		if($customer_id)
		{
			...
			$data['customer_points'] = $cust_info->points;			
		  	$data['sales_until_discount'] = ($this->config->item('number_of_sales_for_discount') ? $this->config->item('number_of_sales_for_discount') : 0) - (float)$cust_info->current_sales_for_discount;
		...
		/**
		 * Here we make the call to FELPLEX API
		 */
		error_log( "Sales ID: ".$data['sale_id_raw']." | Here we begin to declare various: ".( microtime(true) - $reference_time ) );
		$sales_info_general =  $this->sale_id_to_array($data['sale_id_raw']);
		$document_type =  $sales_info_general['custom_fields']['Tipo De Documento'];
		$we_have_anulation =  (int) $sales_info_general['custom_fields']['Anulacion'];
		$we_have_pdf =  $sales_info_general['custom_fields']['PDF'];
		$we_have_ticket =  $sales_info_general['custom_fields']['Ticket'];

		if( empty( $document_type ) ) {
			$this->load->view("sales/receipt",$data);
		}else{
			if( empty($we_have_ticket) && empty($we_have_pdf) ){
				$items = array();

				$searching_discount = array_filter($sales_info_general['cart_items'], function($discount) {
					return $discount['product_id'] === 'Descuento';
				});
				$array_discount = reset($searching_discount);
				if( isset($array_discount) && !empty($array_discount) ){
					$discount_per_item = $array_discount['unit_price']/(count($sales_info_general['cart_items'])-1);
				}
				/**
				 * ITEMS DATA TO SEND
				 */
				foreach($sales_info_general['cart_items'] as $key => $value) {
					
					$product_name = str_replace( '"', '', $value['name'] );
					$item_quantity = $value['quantity'];
					$price = number_format($value['unit_price'], 2, '.', '');
					$discount = $value['discount'];
					$full_name = $sales_info_general['customer_first_name']." ".$sales_info_general['customer_last_name'];
					if( $discount_per_item > 0 ){
						$discount = $discount + (100*$discount_per_item)/$price;
					}
					$tax_code = ($sales_info_general['customer_account_number'] && !empty( $sales_info_general['customer_account_number'] ) ) ? $sales_info_general['customer_account_number'] : 'CF';
					$tax_name =  (!empty($sales_info_general['customer_first_name']) ? $full_name : 'Consumidor Final');
					if( $tax_code != 'CF' ) {
						$tax_name = $this->get_tax_name_by_nit( $tax_code );
					}else{
						$tax_name = 'Consumidor Final';
					}
					$address_street = ($sales_info_general['customer_state'] != '') ? $sales_info_general['customer_state'] : NULL;
					$address_city = ($sales_info_general['customer_city'] != '') ? $sales_info_general['customer_city'] : NULL;
					$address_state = ($sales_info_general['customer_state'] != '') ? $sales_info_general['customer_state'] != '' : NULL;
					$address_zip = ($sales_info_general['customer_zip'] != '') ? $sales_info_general['customer_zip'] != '' : '01001';
					$address_country = ($sales_info_general['customer_country'] != '') ? $sales_info_general['customer_country'] != '' : 'Country';
					$datetime_issue = date("Y-m-d\TH:i:s", strtotime($sales_info_general['sale_time']));
					$total = $sales_info_general['total'];
					$email = $sales_info_general['customer_email'];
					$to_cf = $tax_code === 'CF' ? true : false;
					$external_id = strval($sales_info_general['sale_id']);
					
					if( $product_name === 'Descuento' ){
						continue;
					}
				...
				if( isset($data['payments']) ){
					foreach( $data['payments'] as $payment ){
						if(isset($payment->payment_type) && $payment->payment_type && ( $payment->payment_type=='Tarjeta de crédito' || $payment->payment_type=='Credit Card' )){
							$comment = '<br>Método de Pago: Tarjeta de Crédito <br>Monto: ' . $payment->payment_amount. '<br>No. Tarjeta: ' . substr_replace($payment->truncated_card, 'XXXXXX', 0, 6) . '<br>Aut.: ' . $payment->auth_code . '<br>Ref.: ' . $payment->ref_no;
						}
					}
				}
				
				$felplex_invoice_data = array(
					'type' => 'FACT',
				...
				$sales_info =  json_encode($felplex_invoice_data) ; // -> This info we get to API POS (trait saleTrait)
				$felplex_url = $this->Location->get_info( $this->Employee->get_logged_in_employee_current_location_id() )->location_url_felplex;
				$felplex_entity = $this->Location->get_info( $this->Employee->get_logged_in_employee_current_location_id() )->location_entity_felplex;
				$felplex_api_key = $this->Location->get_info( $this->Employee->get_logged_in_employee_current_location_id() )->location_key_felplex;
		
				error_log( "Sales ID: ".$data['sale_id_raw']." | Here we start the call to Felplex: ".( microtime(true) - $reference_time ) );
				$options = array(
					CURLOPT_URL => $felplex_url . '/api/entity/' . $felplex_entity . '/invoices/await',
					CURLOPT_POST => true,
					CURLOPT_POSTFIELDS => $sales_info,
					CURLOPT_POSTREDIR => 307,
					CURLOPT_RETURNTRANSFER => true,
					CURLOPT_HTTPHEADER => array(
						'Content-Type: application/json',
						'X-Authorization: ' . $felplex_api_key,
					),
				);
		
				$ch = curl_init(); 					//-> Initialize cURL
				error_log( "Sales ID: ".$data['sale_id_raw']." | Here we init called: ".( microtime(true) - $reference_time ) );
				curl_setopt_array($ch, $options); 	//-> Set cURL Options
				$response = curl_exec($ch); 		//-> Execute cURL
				curl_close($ch); 					//-> Close cURL 

				error_log( "Sales ID: ".$data['sale_id_raw']." | Here we receive a response from felplex: ".( microtime(true) - $reference_time ) );
		
				$response_array = json_decode($response, true);
		
				if ( isset($response_array) && $response_array['valid'] == true ) {
					$felplex_pdf = $response_array['invoice_url'];
					$felplex_ticket = str_replace("pdf", "text", $felplex_pdf);

					$sale_data = array(
						'custom_field_3_value' => $felplex_pdf,
						'custom_field_4_value' => $felplex_ticket
					);
					// Update the sale with the invoice
					error_log( "Sales ID: ".$data['sale_id_raw']." | Here we are about to save the custom fields: ".( microtime(true) - $reference_time ) );
					if( $this->Sale->update($sale_data, $data['sale_id_raw']) ) {
						error_log( "Sales ID: ".$data['sale_id_raw']." | The custom fields have already been saved: ".( microtime(true) - $reference_time ) );
						echo '
						<script>
						window.open("'. $felplex_ticket .'","_blank");
						</script>';
						$this->load->view("sales/receipt",$data);
					}
		
				}elseif( ( $response_array['valid'] == false ) && !empty( $response_array['errors'] ) ) {
					foreach ($response_array['errors'] as $error) {
						$data['error'] = [ $error[0] ];
					}
					$this->load->view("sales/receipt",$data);
				}
			}elseif( $we_have_anulation && !( empty($we_have_ticket) && empty($we_have_pdf) ) ){
				
				$felplex_working_url = $this->config->item('base_url_felplex');
				$felplex_entity_id = $this->config->item('entity_id_felplex');
				$api_key =  $this->config->item('api_key_felplex');
				$felplex_uui_invoce = basename( $we_have_pdf );
				
				$api_url = $felplex_working_url . '/api/entity/' . $felplex_entity_id . '/invoices/' . $felplex_uui_invoce;
				$options = array(
					CURLOPT_URL => $api_url,
					CURLOPT_CUSTOMREQUEST => 'DELETE',
					CURLOPT_POSTFIELDS => json_encode(array("reason" => 'Anulación de factura')),
					CURLOPT_RETURNTRANSFER => true,
					CURLOPT_HTTPHEADER => array(
						'Content-Type: application/json',
				...
				$response_array = json_decode($response, true);
				if ( isset($response_array) && $response_array['valid'] == true ) {
					$felplex_pdf_nul = $response_array['invoice_url'];
		
					$sale_data = array(
						'custom_field_3_value' => $felplex_pdf_nul
					);
					// Update the sale with the invoice
					if( $this->Sale->update($sale_data, $data['sale_id_raw']) ) {
						echo '
						<script>
						window.open("'. $felplex_pdf_nul .'","_blank");
						</script>';
						$this->load->view("sales/receipt",$data);
					}
	...
	function cash_receipt($correlative_id)
	{		
		$receipt_cash = $this->Sale->get_register_cash_added_subtracted_by_id($correlative_id);
		
		foreach($receipt_cash as $cash)
		{
		...
		$data = array_merge($data,$receipt_cash);
		
		$data['override_location_id'] = $cash['location_id'];
		
		$data['receipt_title']= lang('sales_cash_receipt');
		
		$data['transaction_time']=$cash['date'];
		
		$data['register_name'] = $this->Register->get_register_name($cash['register_id']);
	
		$data['employee']=$cash['full_name'];
		$data['employee_firstname']=$cash['full_name'];
		$data['amount']=$cash['amount'];
		$data['note']=$cash['note'];

		$data['payment_type']=$cash['payment_type'];

		$data['correlative']=$cash['correlative'];

		$this->load->view("sales/cash_receipt",$data);
	}
	...
	function delete($sale_id)
	{
		...
		/**
		 * Anulación de factura en FELPLEX
		 */

		$sales_info_general =  $this->sale_id_to_array($sale_id);
		$we_have_pdf =  $sales_info_general['custom_fields']['PDF'];
		...
		if ( isset($response_array) && $response_array['valid'] == true ) {
			...
			// Update the sale with the invoice
			$this->Sale->update($sale_data, $data['sale_id_raw']);
		}
	...
	function bac_void_receipt($bac_success = false, $sale_id = null)
	{
		$data['success'] = false;
		$data['sale_id'] = $sale_id;

		if ($bac_success && $this->Sale->delete($sale_id))
		{
			$data['success'] = true;
			if ($this->input->post('clear_sale'))
			{
	...
	function redeem_discount()
	{
		...
		{
			 $cust_info = $this->Customer->get_info($customer_id);
		   $sales_until_discount = ($this->config->item('number_of_sales_for_discount') ? $this->config->item('number_of_sales_for_discount') : 0) - (float)$cust_info->current_sales_for_discount;
  ...
  function cancel_sale()
  {
	...
	function suspend($suspend_type = 1)
	{
		...
		if($work_order_id)
		{
			...
			{
				$suspended_type_text = lang('common_estimate');				
			}
			else
			{
				$this->load->model('Sale_types');
	...
	function all_register_adds_subtracts($mode)
	{
		if ($mode == 'add')
		{
			$data = array();
			...
			$table_data = $this->Sale->get_all_register_cash_added_subtracted($mode);
			$data['manage_table'] = get_cash_register_manage_table($table_data, $this);
			$data['default_columns'] = $this->Sale->get_register_cash_default_columns();
			$data['selected_columns'] = $this->Employee->get_register_cash_columns_to_display();
			$data['page_title'] = lang('sales_list_register_adds');
			$data['all_columns'] = array_merge($data['selected_columns'], $this->Sale->get_register_cash_displayable_columns());		
			$this->load->view('sales/all_register_adds_subtracts', $data);
		}
		else 
		{
			$data = array();
			...
			$data['default_columns'] = $this->Sale->get_register_cash_default_columns();
			$data['selected_columns'] = $this->Employee->get_register_cash_columns_to_display();
			$data['page_title'] = lang('sales_list_register_subtracts');
			$data['all_columns'] = array_merge($data['selected_columns'], $this->Sale->get_register_cash_displayable_columns());		
			$this->load->view('sales/all_register_adds_subtracts', $data);
		}
	}
	...
	function save_column_prefs_cash_register()
	{
		$this->load->model('Employee_appconfig');
		
		if ($this->input->post('columns'))
		{
			$this->Employee_appconfig->save('cash_register_column_prefs',serialize($this->input->post('columns')));
		}
		else
		{
			$this->Employee_appconfig->delete('cash_register_column_prefs');			
		}
	}
	...
	function reload_table_cash_register($mode){
		$data['controller_name'] = strtolower(get_class());
		$table_data = $this->Sale->get_all_register_cash_added_subtracted($mode);
		echo get_cash_register_manage_table($table_data, $this);
	}
	...
	function call_felplex_to_find_nit() {
		$api_url = $this->input->post('api_url');
		$api_key = $this->input->post('api_key');
		$get_all = (int) $this->input->post('get_all') ? (int) $this->input->post('get_all') : 0;
		$options = array(
			CURLOPT_URL => $api_url,
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_HTTPHEADER => array(
		...
		if( $get_all === 1 ) {
			...
			foreach( $response_array as $item ) {
				$final_response[$count_result] = [
					"nit" => $response_array[$count_result]['tax_code'],
					"name" => $response_array[$count_result]['tax_name'],
				];
			...
			$final_response = [
				"nit" => $response_array[0]['tax_code'],
				"name" => $response_array[0]['tax_name'],
			];
		}
	...
	function get_tax_name_by_nit( $nit ) {
		
		$felplex_url = $this->Location->get_info( $this->Employee->get_logged_in_employee_current_location_id() )->location_url_felplex;
		$felplex_entity = $this->Location->get_info( $this->Employee->get_logged_in_employee_current_location_id() )->location_entity_felplex;
		$felplex_api_key = $this->Location->get_info( $this->Employee->get_logged_in_employee_current_location_id() )->location_key_felplex;
		$api_url = $felplex_url . '/api/entity/' . $felplex_entity . '/find/NIT/' . $nit;
