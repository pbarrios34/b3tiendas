...
<table width="100%" >
    <tr>
        <td colspan="2" valign="top" align="left" style="width:33.33%">
            ...
            <p id="sale_receipt"><?php echo lang( 'receivings_receipt_date' ); ?></p>
			<?php
                $recv_trans_time_init = new DateTime($transaction_time);
                $recv_trans_time_create = $recv_trans_time_init->format('d/m/Y H:i');
            ?>
            <p id="sale_time"><?php echo H($recv_trans_time_create); ?></p>
            <p id="sale_creation"><?php echo lang( 'receivings_creation_date' ); ?></p>
            <?php
                $recv_creation_date_init = new DateTime($rev_creation_date);
                $recv_creation_date_fomat = $recv_creation_date_init->format('d/m/Y H:i');
            ?>
            <p id="sale_time_creation"><?php echo H($recv_creation_date_fomat); ?></p>
...
<table width="100%">
    ...
    <tr><td colspan="<?php echo $col_span; ?>"><b><?php echo lang('common_item_number_expanded','',array(),TRUE); ?></b></td></tr>
	<tr>
		<td>&nbsp;</td>
		<?php if (!$this->config->item('hide_all_prices_on_recv')) { ?>
		<td align="right"><b><?php echo lang('common_current_cost','',array(),TRUE); ?></b></td>	
        <td align="right"><b><?php echo lang('common_previous_cost','',array(),TRUE); ?></b></td>	
		<?php } ?>
		
		<td align="right"><b style="text-align:center">Cantidad<br>Original</b></td>
        <td align="right"><b style="text-align:center">Cantidad<br>Recibida</b></b></td>
		<?php if (!$this->config->item('hide_all_prices_on_recv')) { ?>
			<?php if($discount_exists) { ?>
                <td align="right"><b><?php echo lang('common_discount_percent','',array(),TRUE); ?></b></td>
                <td align="right"><b><?php echo lang('reports_difference','',array(),TRUE); ?></b></td>
                <td align="right"><b><?php echo lang('common_total','',array(),TRUE); ?></b></td>
            <?php } else {?>
                <td align="right"><b><?php echo lang('reports_difference','',array(),TRUE); ?></b></td>
                <td align="right"><b><?php echo lang('common_total','',array(),TRUE); ?></b></td>
		    <?php } ?>
...
if ($item->quantity_received > 0 && $item->name != lang('common_store_account_payment', '', array(), TRUE) && $item->name != lang('common_discount', '', array(), TRUE) && $item->name != lang('common_refund', '', array(), TRUE) && $item->name != lang('common_fee', '', array(), TRUE)) {
    ...
    <!-- invoice items-->
                    ...
                    <tr><td colspan="<?php echo $col_span;?>"><?php echo H($item->item_number);?></td></tr>
...
<tr>
	<td>&nbsp;</td>
            <?php if (!$this->config->item('hide_all_prices_on_recv')) { ?>
                ...
                <td align="right" style="border-collapse: collapse; border-bottom:1px dashed #000000;">
                    <?php echo ($mode == 'transfer' && !$see_cost_price) ? "" : to_currency($item->cost_price,10); ?>
                </td>
            <?php } ?>

                <td align="right"><?php echo to_quantity($item->quantity); ?></td>
                <td align="right"><?php echo to_quantity($item->quantity_received); ?></td>
            ...
            <?php if (!$this->config->item('hide_all_prices_on_recv')) { ?>
                ...
                <td align="right">
                    ...
                    ?>
                    <?php if (abs($item->quantity_received) <=0) 
                    {
						$tq = to_currency($item->unit_price * $item->quantity - $item->unit_price * $item->quantity * $item->discount / 100, 10);
						// echo ($mode == 'transfer' && !$see_cost_price) ? "" : "Supuesto a Pagar: ".$tq;
					}
					else if (abs($item->quantity_received) > 0)
					{
						echo ($cart->suspended == 1) ? "Supuesto a Pagar: ".to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100, 10) : to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100, 10);
					}
					?>
                </td>
        ...
        <tr><td colspan="8"><hr style="margin:0;padding:0;"></td></tr>
<?php } ?>
