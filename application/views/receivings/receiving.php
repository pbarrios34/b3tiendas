...
$is_complete = ( ( intval($cart->suspended) === 0 ) && ( $cart->receiving_id !== NULL ) );
...
$there_is_someone_editing = $this->Receiving->there_is_someone_editing( $receiving_id );
$employee_editing = $this->Employee->get_logged_in_employee_info()->person_id;
$is_the_same_employee = false;
if( $employee_editing == $there_is_someone_editing['person_editing'] ){
	$is_the_same_employee = true;
}
?>
<style>
...
<div class="main-content" style="<?php echo $there_is_someone_editing['result'] ? ($is_the_same_employee ? 'display:none' : '') : 'display:none' ?>">
	<div id="sales_page_holder">
		<div class="row register">
			<div class="register-box register-items register-right register-items-form paper-cut">
				<div class="item-form">
					<h3>En estos momentos no es posible editar el recibo, ya que esta siendo editado por: <?php echo $this->Employee->get_info( $there_is_someone_editing['person_editing'] )->full_name ?></h3>
					<!-- Receive  Top Buttons  -->
					<div class="sale-buttons">
							<?php echo form_open("receivings/cancel_receiving", array('id' => '__cancel_sale_form', 'autocomplete' => 'off')); ?>

							<a href="" class="btn btn-cancel" id="__cancel_sale_button">
...
<div class="row register" style="<?php echo $there_is_someone_editing['result'] ? ($is_the_same_employee ? '' : 'display:none') : '' ?>">
		...
		<div class="register-box register-items-form">
			...
			<div class="item-form">
				...
				<div class="input-group contacts  register-input-group">
					...
					<?php if (($this->Employee->has_module_action_permission('receivings', 'add_receiving_suspended_items', $this->Employee->get_logged_in_employee_info()->person_id) && $cart->get_previous_receipt_id()) || (!$cart->get_previous_receipt_id())) { ?>
						<?php
								if( $is_complete || $is_reactivated ) {	?>
									<span style="background-color:gray" class="input-group-addon">
										<?php echo anchor("#", "<i class='icon ti-pencil-alt'></i>", array('class' => '', 'title' => '', 'id' => '', 'style' => $styles_disable_action)); ?>
									</span>
								<?php	} else {	?>
									<span class="input-group-addon">
					...
					<?php
						if( $is_complete || $is_reactivated ) {	?>
							<input type="text" id="item" class="add-item-input pull-left keyboardTop" placeholder="<?php echo lang('common_start_typing_item_name'); ?>" readonly/>
						<?php	} else {	?>
							<input type="text" id="item" name="item" 
							<?php 
							if (!$this->Employee->has_module_action_permission('receivings', 'add_receiving_suspended_items', $this->Employee->get_logged_in_employee_info()->person_id) && $cart->get_previous_receipt_id()) 
							{
								echo 'disabled="disabled"';
							} 
							elseif (($this->Employee->has_module_action_permission('receivings', 'add_receiving_suspended_items', $this->Employee->get_logged_in_employee_info()->person_id) && $cart->get_previous_receipt_id()) || (!$cart->get_previous_receipt_id())) 
							{	 
							...
							?> 
							class="add-item-input pull-left keyboardTop" placeholder="<?php echo lang('common_start_typing_item_name'); ?>" data-title="<?php echo lang('common_item_name'); ?>">
						<?php	}
					?>
					...
					<div class="input-group-addon register-mode <?php echo $mode; ?>-mode dropdown" style="<?php echo ($is_complete || $is_reactivated) ? 'background-color: gray;border: 1px solid #D7DCE5;' : '' ?>">
						<?php
							if( $is_complete || $is_reactivated ) {	?>
								<?php echo anchor("#", "<i style='color:white;' class='icon ti-shopping-cart'></i>" . $modes[$mode], array('style' => $styles_disable_action.'color:white;','class' => '', 'title' => '', 'id' => '', 'data-target' => '#', 'data-toggle' => '', 'aria-haspopup' => '', 'role' => '', 'aria-expanded' => '')); ?>
							<?php	} else {	?>
					...
					<?php if (($this->Employee->has_module_action_permission('receivings', 'add_receiving_suspended_items', $this->Employee->get_logged_in_employee_info()->person_id) && $cart->get_previous_receipt_id()) || (!$cart->get_previous_receipt_id())) { ?>
						<span class="input-group-addon grid-buttons <?php echo $mode == 'store_account_payment' ? 'hidden' : ''; ?>" style="<?php echo ($is_complete || $is_reactivated) ? 'background-color: gray;border: 1px solid #D7DCE5;' : '' ?>">
							<?php
								if( $is_complete || $is_reactivated ) {	?>
									<?php echo anchor("#", "<i style='color:white;' class='icon ti-layout'></i> " . lang('common_show_grid'), array('style' => $styles_disable_action.'color:white;', 'class' => 'none show-grid', 'title' => lang('common_show_grid'))); ?>
								<?php	} else {	?>
					...
					<table id="register" class="table table-hover">
						...
						<thead>
							<tr class="register-items-header">
								...
								<th class="item_sort_able sales_previous_cost <?php echo $this->cart->sort_column && $this->cart->sort_column == 'unit_price'? ($this->cart->sort_type=='asc'?"ion-arrow-down-b":"ion-arrow-up-b"):"";?>"><?php echo lang('common_previous_cost'); ?></th>
								...
								<th class="item_sort_able sales_quantity_received <?php echo $this->cart->sort_column && $this->cart->sort_column == 'quantity_received'? ($this->cart->sort_type=='asc'?"ion-arrow-down-b":"ion-arrow-up-b"):"";?>"><?php echo lang('common_quantity_received'); ?></th>
								...
								<th class="item_sort_able sales_difference <?php echo $this->cart->sort_column && $this->cart->sort_column == 'difference'? ($this->cart->sort_type=='asc'?"ion-arrow-down-b":"ion-arrow-up-b"):"";?>"><?php echo lang('reports_difference'); ?></th>
						...
						if (count($cart_items) == 0) { ?>
							...
							<tbody class="register-item-content" data-line="<?php echo $line; ?>">
								<tr class="register-item-details">
									<?php if (!$this->Employee->has_module_action_permission('receivings', 'remove_receiving_suspended_items', $this->Employee->get_logged_in_employee_info()->person_id) && $cart->get_previous_receipt_id()) { ?>
										<td class="text-center"></td>
									<?php } elseif (($this->Employee->has_module_action_permission('receivings', 'remove_receiving_suspended_items', $this->Employee->get_logged_in_employee_info()->person_id) && $cart->get_previous_receipt_id()) || (!$cart->get_previous_receipt_id())) { ?>
										<?php
											if( $is_complete || $is_reactivated ) {	?>
												<td class="text-center"> <?php echo anchor("#", '<i class="icon ion-android-cancel"></i>', array('class' => '', 'style' => $styles_disable_action)); ?> </td>
											<?php	} else {	?>
									...
									<td>
										<?php
											if( $is_complete || $is_reactivated ) {	?>
												<a <?php echo $disable_action ?>><?php echo H($item->name).($item->variation_name ? '<span class="show-collpased" style="display:none">  ['.$item->variation_name.']</span>' : ''); ?><?php echo $item->size ? ' (' . H($item->size) . ')' : ''; ?></a>
											<?php	} else {	?>
									...
									<td class="text-center">
										...
										?>
										<?php if ($this->Employee->has_module_action_permission('receivings', 'can_edit_new_cost', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
											<?php
												if( $is_complete ) {	?>
													<a <?php echo $disable_action; ?>><?php echo to_currency($item->unit_price, 10); ?></a>
												<?php	} else {	?>
									...
									<td class="text-center">
										<?php if ($this->Employee->has_module_action_permission('receivings', 'can_edit_quantity_purchased', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
											<?php
												if( $is_complete || $is_reactivated ) {	?>
													<a <?php echo $disable_action; ?>><?php echo to_quantity($mode == "transfer" ? abs($item->quantity) : $item->quantity); ?></a>
												<?php	} else {	?>
									...
									<td class="text-center">
										<?php if ($this->Employee->has_module_action_permission('receivings', 'can_edit_quantity_received', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
											<?php
												if( $is_complete ) {	?>
													<a <?php echo $disable_action; ?>><?php echo to_quantity($mode == "transfer" ? abs($item->quantity_received) : $item->quantity_received); ?></a>
												<?php	} else {	?>
													<a href="#" id="quantity_received_<?php echo $line; ?>" class="xeditable edit-quantity_received" data-type="text" data-validate-number="true" data-value="<?php echo H(to_quantity($mode == "transfer" ? abs($item->quantity_received) : $item->quantity_received)); ?>" data-pk="1" data-name="quantity_received" data-url="<?php echo site_url('receivings/edit_item/' . $line); ?>" data-title="<?php echo lang('common_quantity_received') ?>"><?php echo to_quantity($mode == "transfer" ? abs($item->quantity_received) : $item->quantity_received); ?></a>
												<?php	}
											?>
										<?php } else { ?>
											<?php echo to_quantity($item->quantity_received); ?>
										<?php }	?>
									</td>
									...
									<td class="text-center">
										<?php if ($line !== $line_for_flat_discount_item && $this->Employee->has_module_action_permission('receivings', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
											<?php
												if( $is_complete ) {	?>
													<a <?php echo $disable_action; ?>><?php echo to_quantity($item->discount); ?>%</a>
												<?php	} else {	?>
									...
									<td class="text-center">
										...
										?>
											<?php if ($item->quantity_received === NULL) { ?>
												<?php
													echo to_currency(($item->unit_price * $item->quantity - $item->unit_price * $item->quantity * $item->discount / 100) - ($item->cost_price * $item->quantity - $item->cost_price * $item->quantity * $item->discount / 100));
												?>
											<?php } elseif ($item->quantity_received >= 0) { ?>
												<?php
													echo to_currency(($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100) - ($item->cost_price * $item->quantity_received - $item->cost_price * $item->quantity_received * $item->discount / 100));
												?>
											<?php } 
											/*echo to_currency(($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100) - ($item->cost_price * $item->quantity_received - $item->cost_price * $item->quantity_received * $item->discount / 100));*/
											?>
									...
									<td class="text-center">
										...
										?>
											...
											<?php } elseif ($item->quantity_received >= 0) { ?>
												...
												<?php if ($items_module_allowed) { ?>
													<?php
														if( ($is_complete || $is_reactivated) && (!$this->Employee->has_module_action_permission('receivings', 'can_edit_new_cost', $this->Employee->get_logged_in_employee_info()->person_id)) ) {	?>
															<a <?php echo $disable_action; ?>><?php echo to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100); ?></a>
														<?php	} else {	?>
															<?php
																if( $cart->get_mode() == 'purchase_order' ) {	?>
																	<a <?php echo $disable_action; ?>><?php echo to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100); ?></a>
																<?php	} else {	
																	if( $is_complete ){	?>
																		<a <?php echo $disable_action; ?>><?php echo to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100); ?></a>
																	<?php	}else{	?>
																		<a href="#" id="total_<?php echo $line; ?>" class="xeditable" data-type="text" data-validate-number="true" data-pk="1" data-name="total" data-value="<?php echo H(to_currency_no_money($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100)); ?>" data-url="<?php echo site_url('receivings/edit_line_total/' . $line); ?>" data-title="<?php echo lang('common_total') ?>"><?php echo to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100); ?></a>
																<?php	}	?>
																<?php	}
															?>
														<?php	}
													?>
												<?php } else {
													echo to_currency($item->unit_price * $item->quantity_received - $item->unit_price * $item->quantity_received * $item->discount / 100);
												}?>
									...
									<td colspan="5">
										<dl class="register-item-extra-details dl-horizontal">
											...
											<dd class="list">
												<?php
													if( $is_complete || $is_reactivated ) {	?>
														<a <?php echo $disable_action; ?>><?php echo character_limiter(H($item->serialnumber), 50) ? character_limiter(H($item->serialnumber), 50) : lang('common_empty') ; ?></a>
													<?php	} else {	?>
											...
											?>
												...
												<?php if ($this->Employee->has_module_action_permission('sales', 'edit_sale_price', $this->Employee->get_logged_in_employee_info()->person_id)) {	?>
													...
													<dd>
														<?php
															if( $is_complete || $is_reactivated ) {	?>
																<a <?php echo $disable_action; ?>><?php echo to_currency_no_money($item->selling_price, 10) ?></a>
															<?php	} else {	?>
											...
											<dd class="">
												<?php
													if( $is_reactivated ) {	?>
														<a <?php echo $disable_action; ?>><?php echo character_limiter(H($supplier_name), 50); ?></a>
													<?php	} else {	?>
														<a href="#" id="supplier_<?php echo $line; ?>" data-name="supplier" data-type="select" data-pk="1" data-url="<?php echo site_url('receivings/edit_item_supplier/' . $line); ?>" data-title="<?php echo H(lang('common_supplier')); ?>"><?php echo character_limiter(H($supplier_name), 50); ?></a>
													<?php	}
												?>
											</dd>
											...
											<?php if ( (int)$this->Item->get_info( $item->item_id )->expiration ) { 
												$format_date = strftime('%A, %d de %B de %Y', strtotime('today'));
												?>
												<dt><?php echo lang('common_expire_date'); ?></dt>
												<?php
													if( $is_reactivated ) {	?>
														<dd <?php echo $disable_action; ?>><b><a href="#" id="expire_date_<?php echo $line; ?>" class="expire_date" data-type="combodate" data-template="<?php echo get_js_date_format(); ?>" data-pk="1" data-name="expire_date" data-value="<?php echo date('Y-m-d', strtotime($item->expire_date)); ?>" data-url="<?php echo site_url('receivings/edit_item/' . $line); ?>" data-title="<?php echo H(lang('common_expire_date')); ?>"><?php echo $format_date; ?></a></b></dd>
													<?php	} else {	?>
														<dd><b><a href="#" id="expire_date_<?php echo $line; ?>" class="expire_date" data-type="combodate" data-template="<?php echo get_js_date_format(); ?>" data-pk="1" data-name="expire_date" data-value="<?php echo date('Y-m-d', strtotime($item->expire_date)); ?>" data-url="<?php echo site_url('receivings/edit_item/' . $line); ?>" data-title="<?php echo H(lang('common_expire_date')); ?>"><?php echo $format_date; ?></a></b></dd>
													<?php	}
												?>
											<?php } ?>
											<dt class="visible-lg">
		...
		<div class="sale-buttons">
			...
			<div class="btn-group">
				...
				<ul class="dropdown-menu sales-dropdown" role="menu">
					...
					<?php if ($this->Employee->has_module_action_permission('receivings', 'show_receiving_custom_fields', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
						<li>
							<?php echo anchor(
			...
			<?php if (count($cart_items) > 0) { ?>
				...
				if (!$cart->get_previous_receipt_id() || $cart->suspended) { ?>
					<?php if ($this->Employee->has_module_action_permission('receivings', 'suspend_receiving', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
		...
		<?php } else {  ?>
			<?php if (isset($supplier)) {  ?>
				...
				<div class="customer-badge">
					...
					<?php } ?>
						...
						<?php	
							...
							if( $this->Employee->has_module_action_permission('receivings', 'show_can_deleted_supplier', $this->Employee->get_logged_in_employee_info()->person_id) && !$is_complete ) {
								echo anchor("suppliers/view/$supplier_id/1", '<i class="ion-ios-compose-outline"></i>',  array('id' => 'edit_supplier', 'class' => 'btn btn-edit btn-primary pull-right', 'title' => lang('receivings_update_supplier'))) . '';
							}
						}
						?>
				...
				<div class="customer-action-buttons btn-group btn-group-justified ">
					...
					<?php 
						if( $this->Employee->has_module_action_permission('receivings', 'show_can_deleted_supplier', $this->Employee->get_logged_in_employee_info()->person_id) && !$is_complete ) {
							echo '' . anchor("receivings/delete_supplier", '<i class="ion-close-circled"></i> ' . lang('common_detach'), array('id' => 'delete_supplier', 'class' => 'btn')); 
						}
					?>
				</div>
			<?php } else {  ?>
	...
	<div class="register-box register-summary paper-cut">
			...
			<?php if ($this->Employee->has_module_action_permission('receivings', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
				...
				<li class="list-group-item global-discount-group">
					...
					<div class="value pull-right"><span id="TEST"><?php echo $symbol; ?></span>
						<?php
							if( $is_complete ) {	?>
								<a href="#" style="<?php echo $styles_disable_action." border-bottom: dashed 1px gray;"; ?>"><?php echo H(lang('receivings_set_discount')); ?></a>
							<?php	} else {	?>
		...
		?>
					...
					<?php foreach ($payments as $payment_id => $payment) { ?>
						<li class="list-group-item">
							<span class="key">
								<?php
									if( $is_complete ) {	?>
										<?php echo anchor("#", '<i class="icon ion-android-cancel"></i>', array('class' => '', 'id' => '', 'style' => $styles_disable_action)); ?>
									<?php	} else {	?>
			...
			if ($mode != "transfer") { ?>
				<?php if ($supplier_required_check) { ?>
					<div class="add-payment">
						...
						<div class="row">
							<div id="create_invoice_holder" class="create_invoice_holder col-md-6;">
								<div class="text-center">
								<?php echo form_label(lang('common_create_invoice'),array('class'=>'control-label wide'), $value="", $extra=$disable_action); ?>
								...
								<label style="padding-left: 10px; margin-top:0px;<?php echo $styles_disable_action; ?>"><span></span></label>
							</div>
							</div>
						</div>
						...
						<?php
						if (!$selected_payment) {
							$selected_payment = lang( 'common_store_account' );
						}
						?>
						<?php foreach ($payment_options as $key => $value) {
							$active_payment =  ($selected_payment == $value) ? "active" : "";
						?>
							<a class="btn btn-pay select-payment <?php echo $active_payment; ?>" style="<?php echo "pointer-events: none;cursor: default;". ( $active_payment ? 'background-color: gray;border-color: #d2d2d2;' : '' ); ?>">
								<?php echo H($value); ?>
							</a>
						<?php } ?>
						...
						<div class="input-group add-payment-form">
							...
							<?php
								if( $is_complete ) {	?>
									<?php echo form_input(array('name' => 'amount_tendered', 'id' => 'amount_tendered', 'value' => to_currency_no_money($amount_due), 'class' => 'add-input numKeyboard form-control '.(!$has_cost_price_permission ? 'hidden' : ''), 'data-title' => lang('common_payment_amount')), $value="", $extra="readonly");	?>
								<?php	} else {	?>
			...
			<?php for ($k = 1; $k <= NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k++) { ?>
						...
						$can_edit_custom_field = $this->Employee->has_module_action_permission( 'receivings', 'show_can_edit_custom_field_'.$k, $this->Employee->get_logged_in_employee_info(  )->person_id );
						if ($custom_field !== FALSE) {
							...
							<div class="custom_field_block <?php echo "custom_field_${k}_value"; ?>">
								...
								<?php if ($this->Receiving->get_custom_field($k, 'type') == 'checkbox') { ?>
									...
									<?php echo form_checkbox("custom_field_${k}_value", '1', (bool) $cart->{"custom_field_${k}_value"}, "id='custom_field_${k}_value' class='custom-fields-checkbox customFields' $required_text".( ( $can_edit_custom_field && !$is_complete ) ? "" : "disabled" )); ?>
								...
								<?php } elseif ($this->Receiving->get_custom_field($k, 'type') == 'date') { ?>
									...
									),$value="", ( ($can_edit_custom_field && !$is_complete) ? "" : $extra="disabled" )); ?>
								...
								<?php } elseif ($this->Receiving->get_custom_field($k, 'type') == 'dropdown') { ?>
									...
									echo form_dropdown("custom_field_${k}_value", $select_options, $cart->{"custom_field_${k}_value"}, 'class="form-control custom-fields-select customFields" '.$required_text.( ($can_edit_custom_field && !$is_complete) ? "" : "disabled" )); ?>
								...
								<?php } elseif ($this->Receiving->get_custom_field($k, 'type') == 'image' || $this->Receiving->get_custom_field($k, 'type') == 'file') {
									echo form_input(
										...
										$value = NULL,
										$extra = ($cart->{"custom_field_${k}_value"} ? "" : " ".$required_text) . ( ($can_edit_custom_field && !$is_complete) ? "" : " disabled" )
									);
									...
									if ($cart->{"custom_field_${k}_value"} && $this->Receiving->get_custom_field($k, 'type') == 'image') {
										...
										echo "<div class='delete-custom-image-recv'><a href='" . site_url('receivings/delete_custom_field_value/' . $k) . "' ".(($can_edit_custom_field && !$is_complete) ? "" : $disable_action) .">" . lang('common_delete') . "</a></div>";
									...
									echo form_input(array(
										...
										($required ? $required_text : $required_text) => ($required ? $required_text : $required_text)
									), $value="", ( ($can_edit_custom_field && !$is_complete) ? "" : $extra="readonly" ) ); ?>
								<?php } ?>
								<?php echo '</div>' ?>
						<?php } //end if ?>
					...
					<div class="change-date">
						...
						<div id="change_cart_date_picker" class="input-group date datepicker">
							...
							<?php echo form_input(array(
								...
								'size' => '8',
								'class' => 'form-control',
								'value' => date(get_date_format() . ' ' . get_time_format(), time()),
							)); ?>
						</div>
...
<script type="text/javascript" language="javascript">
	...
	$(document).ready(function() {
		...
		$(".expire_date").click(function(e) {
			e.preventDefault();
			setTimeout(() => {
				let selectElement = document.querySelector(".register-item-extra-details .popover select.month");
		...
		$("#__cancel_sale_button").click(function(e) {
			e.preventDefault();
			$('#__cancel_sale_form').ajaxSubmit({
				target: "#register_container",
				beforeSubmit: receivingsBeforeSubmit
			});
		});
