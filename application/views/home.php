...
$this_employee_can_confirm_items_exp = $this->Employee->has_module_action_permission('reports', 'can_confirm_items_quantity_exp', $this->Employee->get_logged_in_employee_info()->person_id);
?>
	...
	<?php if(!$this->config->item('hide_expire_dashboard') && count($expiring_items) > 0) { ?> 
		...
		<table class="table">
			<tr>
				...
				<?php
					if( $this_employee_can_confirm_items_exp ){	?>
						<th><?php echo lang('common_items_exp')?></th>
					<?php	}
				?>
			</tr>
			
			<?php foreach($expiring_items as $eitem) {
				$recv_id = $eitem['receiving_id'];
				$item_id = $this->Receiving->get_item_id_by_product_id( $eitem['product_id'] );
				$have_quantity_confirmed = $this->Receiving->have_quantity_confirmed( $recv_id, $item_id, $eitem['quantity_expiring'] );
				$fecha_actual = date('Y-m-d');
				?>
					<tr style="<?php echo $have_quantity_confirmed ? 'display:none;' : '' ?><?php echo ($fecha_actual >= $eitem['expire_date']) ? 'background: rgba(255, 0, 0, .25);' : '' ?>">
						...
						<?php
							if( $this_employee_can_confirm_items_exp ){	?>
								<td id="confirm_quantity_exp" class="text-center">
									<a href="/index.php/Receivings/quick_modal?expire_date=<?php echo $eitem['expire_date']; ?>&item_id=<?php echo $item_id; ?>&employee_id=<?php echo $this->Employee->get_logged_in_employee_info()->id; ?>" class="btn btn-primary submit_button btn-large" data-toggle="modal" data-target="#myModalDisableClose" style="<?php echo ($fecha_actual >= $eitem['expire_date']) ? '' : 'pointer-events: none; cursor: default; background: gray;' ?>">Confirmar</a>
								</td>
							<?php	}
						?>
					</tr>
