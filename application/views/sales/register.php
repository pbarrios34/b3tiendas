			...
			<div class="register-box register-items-form">
				...
				<div id="itemForm" class="item-form">
					...
					<div class="input-group contacts register-input-group">
						...
						<span class="input-group-addon" <?php echo $we_are_editing ? 'style="background-color: gray;"': ''; ?>>
							<?php
								if( $we_are_editing ){	?>
									<?php echo anchor("#", "<i class='icon ti-pencil-alt'></i> <span class='register-btn-text'>" . lang('common_new_item') . "</span>", array('class' => 'none add-new-item', 'title' => lang('common_new_item'), 'style' => 'pointer-events: none;cursor:', 'tabindex' => '-1')); ?>
								<?php	}else{	?>
									<?php echo anchor("items/view/-1?redirect=sales/index/1&progression=1", "<i class='icon ti-pencil-alt'></i> <span class='register-btn-text'>" . lang('common_new_item') . "</span>", array('class' => 'none add-new-item', 'title' => lang('common_new_item'), 'id' => 'new-item-mobile', 'tabindex' => '-1')); ?>
								<?php	}
							?>
						</span>
						<input type="text" id="item" name="item" <?php echo ($mode == "store_account_payment" || $mode == 'purchase_points' || $we_are_editing) ? 'disabled="disabled"' : '' ?> class="add-item-input pull-left keyboardTop" placeholder="<?php echo lang('common_start_typing_item_name'); ?>" data-title="<?php echo lang('common_item_name'); ?>">
						...
						<div class="input-group-addon register-mode <?php echo H($mode); ?>-mode dropdown" <?php echo $we_are_editing ? 'style="border-color: gray;background: #d3d3d3;"' : '' ?>>
							<?php
								if( $we_are_editing ){	?>
									<?php echo anchor("#", "<i class='icon ti-pencil-alt' style='color: #fff;'></i> <span class='register-btn-text'>" . lang('common_new_item') . "</span>", array('title' => lang('common_new_item'), 'style' => 'color: #fff;pointer-events: none;cursor: default;', 'tabindex' => '-1')); ?>
								<?php	}else{	?>
						...
						<span class="input-group-addon grid-buttons <?php echo $mode == 'store_account_payment' || $mode == 'purchase_points' ? 'hidden' : ''; ?>" <?php echo $we_are_editing ? 'style="border-color: gray;background: #d3d3d3;"' : '' ?>>
						<?php
							if( $we_are_editing ){	?>
								<?php echo anchor("#", "<i class='icon ti-layout'></i> " . lang('common_show_grid'), array('style' => 'pointer-events: none;cursor: default;')); ?>
							<?php	}else{	?>
										...
										<tr class="register-item-details">
											...
											?>
												<td class="text-center"> 
													<?php
														if( $we_are_editing ) {
															echo anchor("#", '<i class="icon ion-android-cancel"></i>', array('class' => 'delete-item', 'tabindex' => '-1', 'style' => $styles_disable_action)); 
														}else{
															echo anchor("sales/delete_item/$line", '<i class="icon ion-android-cancel"></i>', array('class' => 'delete-item', 'tabindex' => '-1')); 
														}
													?> 
												</td>
											<?php
											...
											<td>
												...
												<?php
													if( $we_are_editing ){	?>
														<a tabindex="-1" href="#" class="register-item-name" <?php echo $disable_action; ?>><?php echo H($item->name);?></a>
													<?php	}else{	?>
											...
											<td class="text-center" <?php echo $we_are_editing ? $disable_action : ''; ?> >
												<?php
												if (!$cart->suspended || $this->Employee->has_module_action_permission('sales', 'edit_suspended_sale', $this->Employee->get_logged_in_employee_info()->person_id)) {
												?>
													<?php if ($item->product_id != lang('common_integrated_gift_card') && ($item->allow_price_override_regardless_of_permissions || $this->Employee->has_module_action_permission('sales', 'edit_sale_price', $this->Employee->get_logged_in_employee_info()->person_id))) { ?>
														<?php
															if( $we_are_editing ){	?>
																<a href="#" class="xeditable xeditable-price" <?php echo $disable_action; ?>><?php echo to_currency($item->unit_price, 10); ?></a>
															<?php	}else{	?>
											...
											<td class="text-center">
												<?php if ($item->product_id != lang('common_integrated_gift_card') && (!$cart->suspended || $this->Employee->has_module_action_permission('sales', 'edit_suspended_sale', $this->Employee->get_logged_in_employee_info()->person_id))) { ?>
													<?php if ($this->config->item('number_of_decimals_displayed_on_sales_interface')) { ?>
														<?php
															if( $we_are_editing ){	?>
																<a href="#" class="xeditable xeditable-price" <?php echo $disable_action; ?>><?php echo to_currency_no_money($item->quantity, $this->config->item('number_of_decimals_displayed_on_sales_interface')); ?></a>
															<?php	}else{	?>
											...
											<td class="text-center">
												...
												?>
													<?php if ($line !== $line_for_flat_discount_item && $this->Employee->has_module_action_permission('sales', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
														<?php
															if( $we_are_editing ){	?>
																<a href="#" class="xeditable xeditable-price" <?php echo $disable_action; ?>><?php echo to_quantity($item->discount); ?>%</a>
															<?php	}else{	?>
											...
											<td class="text-center">
												...
												?>
													...
													<?php if ($item->allow_price_override_regardless_of_permissions || $this->Employee->has_module_action_permission('sales', 'edit_sale_price', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
														<?php
															if( $we_are_editing ){	?>
																<a href="#" class="xeditable xeditable-price" <?php echo $disable_action; ?>><?php echo to_currency($item->unit_price * $item->quantity - $item->unit_price * $item->quantity * $item->discount / 100); ?></a>
															<?php	}else{	?>
										...
										<tr class="register-item-bottom">
											...
											<td colspan="5">
												<dl class="register-item-extra-details dl-horizontal">
													...
													<?php if ($cart->get_previous_receipt_id()) { ?>
														...
														<?php
															if( $we_are_editing ){	?>
																<a href="#" class="xeditable xeditable-price" <?php echo $disable_action; ?>><?php echo H(to_quantity($item->quantity_received)); ?></a>
															<?php	}else{	?>
													...
													if (property_exists($item, 'quantity_units') && count($item->quantity_units) > 0) { ?>
														...
														<dd class="">
														<?php	if( $we_are_editing ){	?>
															<a href="#" <?php echo $disable_action; ?>><?php echo character_limiter(H($item->quantity_unit_id ? $item->quantity_units[$item->quantity_unit_id] : lang('common_none')), 50); ?></a></dd>
														<?php	}else{	?>
													...
													if (!empty($variation_choices)) { ?>
														...
														<?php	if( $we_are_editing ){	?>
															<a <?php echo $disable_action; ?>><?php echo lang('common_edit'); ?></a>
															<dd class=""><a href="#" <?php echo $disable_action; ?>><?php echo character_limiter(H($item->variation_name), 50); ?></a></dd>
														<?php	}else{	?>
											...
											<?php } ?>
												...
												<?php if (!$this->config->item('hide_description_on_sales_and_recv')) { ?>
													...
													?>
														<dt style="display:none"><?php echo lang('common_stock'); ?></dt>
														<dd style="display:none"><?php echo to_quantity($cur_quantity); ?></dd>
													...
													<?php if ($this->Employee->has_module_action_permission('sales', 'edit_taxes', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
														...
														<dd>
															<?php
																if( $we_are_editing ){	?>
																	<a href="#" class="xeditable xeditable-price" <?php echo $disable_action; ?>><?php echo lang('common_edit_taxes'); ?></a>
																<?php	}else{	?>
...
<div class="col-lg-4 col-md-5 col-sm-12 col-xs-12">
	<div class="register-box register-right">
			...
			<div class="btn-group">
				...
				<ul class="dropdown-menu sales-dropdown" role="menu">
					...
					if ($this->config->item('track_payment_types') && !empty($track_payment_types)) { ?>
						...
						<?php } else {?>
							<li><?php echo anchor(site_url('sales/all_register_adds_subtracts/add'), '<i class="ion-cash"></i> ' . lang('sales_view_add_cash_to_register'), array('class' => '')); ?></li>
							<li><?php echo anchor(site_url('sales/all_register_adds_subtracts/subtract'), '<i class="ion-log-out"></i> ' . lang('common_view_remove_cash_from_register'), array('class' => '')); ?></li>
						<?php }?>
					...
					<?php if ($this->Employee->has_module_action_permission('sales', 'show_sales_custom_fields', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
						<li>
							<?php echo anchor(
								"sales/custom_fields",
			...
			<?php if (count($cart_items) > 0) { ?>
				...
				<?php if ($mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
					...
					<?php if ($this->Employee->has_module_action_permission('sales', 'suspend_sale', $this->Employee->get_logged_in_employee_info()->person_id) && $customer_required_check && $suspended_sale_customer_required_check && !$this->config->item('test_mode')) { ?>
						<div class="btn-group">
							<?php	if( $we_are_editing ){	?>
								<button type="button" class="btn btn-suspended dropdown-toggle" data-toggle="dropdown" aria-expanded="false" style="pointer-events: none; cursor: default; background: #d3d3d3d3; color: #fff; border: gray;">
									<i class="ion-pause"></i>
		...
		<?php if (isset($customer)) { ?>
			...
			<div class="customer-badge">
				...
				<div class="details">
					<?php if (!$this->config->item('hide_customer_recent_sales') && isset($customer)) { ?>
						<?php
							if( $we_are_editing ){	?>
								<a href="#" class="name" <?php echo $we_are_editing ? $disable_action : '' ?>><?php echo character_limiter(H($customer), 30); ?></a>
							<?php	}else{	?>
			...
			<div class="customer-action-buttons  btn-group btn-group-justified " <?php echo $we_are_editing ? 'style="display:none":' : '' ?>>
	...
	<div class="register-box register-summary paper-cut">
		...
		<ul class="list-group" <?php echo $we_are_editing ? 'style="display:none":' : '' ?>>
		...
		<?php if (count($cart_items) > 0) { ?>
						...
						<li class="list-group-item">
							<span class="key">
								...
								?>
									<?php
										if( $we_are_editing ){	?>
											<?php echo anchor("#", '<i class="icon ion-android-cancel"></i>', array( 'style' => $styles_disable_action )); ?>
										<?php	}else{	?>
			...
			<?php if ($customer_required_check) { ?>
				<div class="add-payment">
					...
					?>
						<a tabindex="-1" href="#" class="btn btn-pay select-payment <?php echo $active_payment; ?>" data-payment="<?php echo H($value); ?>" <?php echo $we_are_editing ? 'style="pointer-events: none;cursor: default;background-color: gray;border-color:#000;color:#fff"' : '' ?>>
							<?php echo H($value); ?>
						</a>
					<?php } ?>
					...
					<div class="input-group add-payment-form">
						...
						<?php 
							...
							echo form_hidden('customer_amount_tendered', to_currency_no_money($amount_due));
						?>
						<span class="input-group-addon" <?php echo $we_are_editing ? 'style="background-color: gray;"' : ''; ?>>
							<a href="#" class="" id="add_payment_button" <?php echo $we_are_editing ? 'style="pointer-events: none;cursor: default;background-color: gray;border-color:#000;color:#fff"' : ''; ?>><?php echo lang('common_add_payment'); ?></a>
							<a href="#" class="hidden" id="finish_sale_alternate_button" <?php echo $we_are_editing ? 'style="pointer-events: none;cursor: default;background-color: gray;border-color:#000;color:#fff"' : ''; ?>><?php echo lang('sales_complete_sale'); ?></a>
						</span>
			...
			<div class="comment-block">
				...
				<?php 
					if( $we_are_editing ){
						echo form_textarea(array('name' => 'comment', 'value' => $comment, 'rows' => '2', 'class' => 'form-control', 'placeholder' => lang('common_comments'), 'disabled' => true));
						echo form_checkbox(array(
						'value' => '1',
						...
						echo form_textarea(array('name' => 'comment', 'id' => 'comment', 'value' => $comment, 'rows' => '2', 'class' => 'form-control', 'data-title' => lang('common_comments'), 'placeholder' => lang('common_comments')));
						echo form_checkbox(array(
				...
				if ($custom_field !== FALSE) { 
					...
					<div class="custom_field_block <?php echo "custom_field_${k}_value"; ?>">
						...
						<?php } elseif ($this->Sale->get_custom_field($k, 'type') == 'dropdown') { ?>
							...
							array_push($select_options,lang('common_please_select'));
							if( $we_are_editing ){
								echo form_dropdown("custom_field_${k}_value", $select_options, $cart->{"custom_field_${k}_value"}, 'class="form-control custom-fields-select customFields" '.$required_text.' disabled');
							}else{
						...
						<?php } elseif ($this->Sale->get_custom_field($k, 'type') == 'image' || $this->Sale->get_custom_field($k, 'type') == 'file') {
							...
							if( $we_are_editing ){
								echo form_input(array(
									...
									'disabled'	=>	$we_are_editing ? true : false
								)); 
							}else{
								echo form_input(array(
									'name' => "custom_field_${k}_value",
	...
	<div class="modal-dialog">
		<div class="modal-content">
					...
					<div class="col-md-12">
						<div class="form-group">
							<div class="input-group">
								...
								<input type="text" class="form-control" id="custom_amount" autocomplete="off" disabled>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="modal-footer">
				...
				<button data-bb-handler="confirm" data-quick_amount="0" type="button" class="btn btn-primary quick_amount" id="collect_amount" disabled><?php echo lang('common_collect'); ?></button>
			</div>
		</div>
	</div><!-- /.modal-content -->
</div><!-- /.modal-dialog -->
...
<script type="text/javascript" language="javascript">
	...
	$(document).ready(function() {
		...
		<?php if (!isset($customer)) { ?>
			...
			?>
				...
				$('#customer').autocomplete({
					...
					minLength: 0,
					select: function (event, ui) {
						if (ui.item.value === 'QUICK_ADD|' + $('#customer').val()) {
							event.preventDefault();
							$('#new-customer').click( );
							setTimeout(() => {
								let inputNit = '';
								let valueInputNit = document.querySelector('#customer').value;
								let nitPattern = /(^[0-9]{6,12}$)|(^[0-9]{1,6}([0-9]?)(-?[0-9kK]){1}$)|(^CF$)/;

								if( valueInputNit == '' || valueInputNit == 'cf' || valueInputNit == 'c/f' || valueInputNit == 'C/F' || valueInputNit == 'Cf' || valueInputNit == 'cF' ) {
									valueInputNit = 'CF';
								}

								if (nitPattern.test(valueInputNit)) {
									...
									$.ajax({
										...
										method: "POST",
										data: { api_url: api_url, api_key: api_key, get_all: 0 },
										success: function(response) {
											let response_array = JSON.parse(response);

											if( response_array.nit !== '' ) {
												let inputName__e =  response_array.name;
												let inputNit__e =  response_array.nit;
												let inputName = document.querySelector('#first_name');
												let inputNit = document.querySelector('#sales_nit_id');

												inputNit.value = inputNit__e;
												inputName.value = inputName__e;
											}
										},
									...
									inputNit = document.querySelector('#first_name');
									inputNit.value = valueInputNit;
								}
							}, 1000);
						} else {
		...
		function selectPayment(e) {
			...
			let paymentType = e.target.getAttribute('data-payment');
			let quantity_field = document.querySelector('#amount_tendered');
			quantity_field.disabled = true;
			if( ( paymentType == 'Efectivo' ) || ( paymentType == 'Tarjeta de regalo' ) ) {
	...
	$(".btn-pay").dblclick(function(){
		let type_payment = $(".select-payment.active").data('payment');
		let modal_styles = "margin-bottom:15px; pointer-events: none;cursor: default;";
		$("#custom_amount").val('');
		if( type_payment == 'Efectivo' ){
			modal_styles = "margin-bottom:15px;";
			$("#custom_amount").removeAttr("disabled");
			$("#collect_amount").removeAttr("disabled");
		}else{
			$("#custom_amount").attr("disabled", "disabled");
			$("#collect_amount").attr("disabled", "disabled");
		}
		...
		$.each($possible_amount, function($index, $value) {
			...
			$html += '<div class="col-md-3" style="'+modal_styles+'">';
