...
class Register extends MY_Model
{
	...
	*/
	...
	function get_last_register_log_audit($location_id = FALSE)
	{
		...
		$this->db->select('rla.id as correlative');
		...
		$this->db->order_by('rla.id DESC');
		$this->db->limit(1);
		$query = $this->db->get();
		
		if ($row = $query->row_array())
		{
			return $row['correlative'];
		}
		return FALSE;
	...
	function get_current_register_log()
	{
	 ...
	 {
 		$this->db->select("r.name as register_name, open_person.first_name as open_first_name, open_person.last_name as open_last_name, close_person.first_name as close_first_name, close_person.last_name as close_last_name, 
		rlp.*,rl.*, (close_amount - open_amount - payment_sales_amount - total_payment_additions + total_payment_subtractions) as difference, l.name as location");
 		$this->db->from('register_log as rl');
 		$this->db->join('people as open_person', 'rl.employee_id_open=open_person.person_id');
 		$this->db->join('people as close_person', 'rl.employee_id_close=close_person.person_id', 'left');
 		$this->db->join('registers as r', 'r.register_id = rl.register_id');
 		$this->db->join('register_log_payments as rlp', 'rl.register_log_id = rlp.register_log_id');
		 $this->db->join('locations as l', 'l.location_id = r.location_id');
 		$this->db->where('rl.register_log_id', $id);
	...
	function get_previous_register_close_amount( $register_id, $register_log_id ) {
		$this->db->select('register_log_id');
		...
		$this->db->order_by('register_log_id DESC');
		$this->db->limit(1);
		$id_log_previous_register_close = $this->db->get()->row_array()['register_log_id'];
		$previous_register_close_detail = $this->get_register_log( $id_log_previous_register_close )[0];
		$previous_register_close_amount = $previous_register_close_detail->close_amount;
		
		return $previous_register_close_amount;
	}
