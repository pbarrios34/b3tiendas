...
class Detailed_receivings extends Report
{
	...
	public function getDataColumns()
	{
		...
		array('data'=>lang('reports_difference'), 'align'=>'right'), 
	...
	function getOutputData()
	{
		...
		foreach(isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary']:$report_data as $key=>$row)
		{
			...
			$summary_data[$key] = 
			array( array('data'=>anchor('receivings/receipt/'.$row['receiving_id'], '<i class="ion-printer"></i>', array('target' => '_blank')).' '.anchor('receivings/edit/'.$row['receiving_id'], '<i class="ion-document-text"></i>', array('target' => '_blank')).' '.anchor('receivings/edit/'.$row['receiving_id'], lang('common_edit').' '.$row['receiving_id'], array('target' => '_blank')).' ['.anchor('items/generate_barcodes_from_recv/'.$row['receiving_id'], lang('common_barcode_sheet'), array('target' => '_blank', 'class' => 'generate_barcodes_from_recv')).' / '.anchor('items/generate_barcodes_labels_from_recv/'.$row['receiving_id'], lang('common_barcode_labels'), 
	...
	function get_report_details_recv($ids, $export_excel=0)
	{
		...
		foreach($res as $key=>$drow) {	
			...
			$details_data_row[] = array('data'=>to_currency($this->Receiving->get_actual_cost_in_recv( $ids[0], $drow['item_id'] )), 'align'=>'left');
			$details_data_row[] = array('data'=>to_currency($this->Receiving->get_previous_cost_in_recv( $ids[0], $drow['item_id'] )), 'align'=>'left');
