	...
	*/
	...
	function get_displayable_columns()
	{
		...
		$return  = array(
			...
			'previous_cost'  =>  					array('sort_column' => 'previous_cost', 'label' => lang('common_previous_cost'),'format_function' => 'to_currency_and_edit_item_price','data_function' => 'item_id_data_function', 'html' => TRUE),
		);
  ...
  */
  function add_variation_data(&$suggestions, $search, $price_field)
  {
			...
			if ($this->get_custom_field($k)) 
			{
				...
				foreach($by_custom_field->result() as $row)
				{
					$data = array(
						...
						'label' => $row->name. ': '.$row->$custom_field.' - '.($price_field ? to_currency($row->$price_field) : ''),
						'secondary_suppliers' => $this->get_secondary_suppliers($row->item_id)->result_array(),
	...
	*/
	...
	function get_secondary_categories_full_route($item_id){
		$this->db->select("GROUP_CONCAT(c.name SEPARATOR '|' ) as sub_category");
		$this->db->from('items_secondary_categories as sc');
		$this->db->join('categories as c', 'sc.category_id = c.id');
		$this->db->where('sc.item_id',$item_id);
		$this->db->group_by('sc.item_id');
		return $this->db->get();
	}
	...
	function update_price_after_update_inventory($item_id,$cost_price,$unit_price){
		$this->db->where('item_id',$item_id);
		$this->db->update('items',array('item_id' => $item_id, 'cost_price' => $unit_price, 'previous_cost' => $cost_price));
	}
