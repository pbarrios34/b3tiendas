...
class Receiving extends MY_Model
{
	...
	function save ($cart)
	{
		...
		{
			$receivings_data['rev_creation_date'] = date('Y-m-d H:i:s');
			$receivings_data['rev_creation_location_id'] = $this->Employee->get_logged_in_employee_current_location_id();
			$previous_receiving_items = array();
			$this->db->insert('receivings',$receivings_data);
			$receiving_id = $this->db->insert_id();
			if( $receiving_id  == 0 ){
				$receiving_id = $this->get_custom_receiving_id(  );
			}
		}
		...
		foreach($items as $line=>$item)
		{
			...
			if ($item->item_id != $store_account_item_id)
			{
				...
				{
					$cost_price = ($cur_item_location_info && $cur_item_location_info->cost_price) ? $cur_item_location_info->cost_price : $item->cost_price;
				}
			}
			...
			} elseif( !is_null( $item->quantity_received ) && ( $item->quantity_received >=0 ) ) {
				$quantity_received = $item->quantity_received;
				$total_quantity_received+=$quantity_received;
			}
			
			$recv_item_subtotal = ( $item->get_subtotal_receivings() === NULL ) ? 0 : $item->get_subtotal_receivings();
			...
			(
				...
				'subtotal' => $recv_item_subtotal == NULL ? 0 : $recv_item_subtotal,
			...
			if (!$cur_item_info->is_service)
			{
				...
				/**
				 * ABOUT $suspend
				 * When receiving is suspend -> $suspended = 1
				 * When receiving is completed -> $suspended = 0
				 */
				$quatity_received = $item->quantity_received !== NULL ? $item->quantity_received : $item->quantity;
				$inventory_to_add = $suspended == 0 ? $quatity_received : 0;
				if ($item->variation_id)
				{
					...
					if ($inventory_to_add !=0)
					{
						...
						$this->Item->update_price_after_update_inventory($item->item_id, $cost_price,$item->unit_price);
						...
						(
							...
							'trans_inventory'=>$quatity_received,
							'location_id'=>$cart->location_id ? $cart->location_id : ($cart->transfer_from_location_id ? $cart->transfer_from_location_id : $this->Employee->get_logged_in_employee_current_location_id()),
	...
	function get_actual_cost_in_recv( $receiving_id, $item_id ) {
		$this->db->select('item_unit_price');
		$this->db->where('receiving_id', $receiving_id);
		$this->db->where('item_id', $item_id);
		$query = $this->db->get('receivings_items');
		return $query->row()->item_unit_price;
	}

	function get_previous_cost_in_recv( $receiving_id, $item_id ) {
		$this->db->select('item_cost_price');
		$this->db->where('receiving_id', $receiving_id);
		$this->db->where('item_id', $item_id);
		$query = $this->db->get('receivings_items');
		return $query->row()->item_cost_price;
	}
	...
	function validate_init_date_recv( $id_recv ){
		$init_date =  new DateTime( $this->get_info($id_recv)->result_array(  )[0]['rev_creation_date'] );
		$complete_date = new DateTime( date("Y-m-d") );

		if( $init_date < $complete_date ){
			return true;
		}else{
			return false;
		}
	...
	function validate_creation_location_recv( $recv_id, $actual_location_id ){
		$receipt_creation_location_id = is_null($this->get_info($recv_id)->result_array(  )[0]['rev_creation_location_id']) ? $actual_location_id : $this->get_info($recv_id)->result_array(  )[0]['rev_creation_location_id'];
		$receipt_actual_location_id = $actual_location_id;
		
		if( $receipt_creation_location_id === $receipt_actual_location_id ){
			return true;
		}else{
			return false;
		}
	...
	function get_custom_receiving_id(  ){
	...
	function there_is_someone_editing( $receiving_id ){
		$this->load->database();
	
		$query = $this->db->select('some_editing, person_editing')
							->from('receivings')
		...
		$recv = array(
			'some_editing' => $result['some_editing'],
			'person_editing' => $result['person_editing']
		);	

		if( $result['some_editing'] && $result['person_editing'] != -1 ){
			$recv['result'] = true;
		}else{
			$recv['result'] = false;
		}
	...
	function get_recv_id_by_expire_date( $product_id, $expire_date, $quantity ) {
		$item_id = $this->get_item_id_by_product_id( $product_id );
		$this->db->select('receiving_id');
		$this->db->from('receivings_items');
		$this->db->where('expire_date', $expire_date);
		$this->db->where('item_id', $item_id);
		$this->db->where('quantity_purchased', $quantity);
		$query = $this->db->get();
	
		if ($query->num_rows() > 0) {
			return $query->row()->receiving_id;
		} else {
			return null;
		}
	}
	...
	function update_items_quantity_exp( $items, $employee_id ) {
		$current_date = date('Y-m-d H:i:s');
		
		foreach( $items as $item ){
			$data = array(
				'items_quantity_exp_confirmed' => $item->quantity_purchased,
				'employee_confirmed_quantity_items_exp' => $employee_id,
				'date_items_quantity_exp_confirmed' => $current_date
			);
			$this->db->where('receiving_id', $item->receiving_id);
	...
	function update_items_quantity_exp_no_equal( $items, $employee_id, $quantity_diff ){
		$current_date = date('Y-m-d H:i:s');
		$diff = $quantity_diff;

		foreach( $items as $item ){
			if( $diff > 0 ){
				$items_quantity_exp_confirmed = ( $item->quantity_purchased-$diff >= 0 ) ? $item->quantity_purchased-$diff : 0;
			}else{
				$items_quantity_exp_confirmed = $item->quantity_purchased;
			}
			$data = array(
				'items_quantity_exp_confirmed' => $items_quantity_exp_confirmed,
				'employee_confirmed_quantity_items_exp' => $employee_id,
				'date_items_quantity_exp_confirmed' => $current_date
			);
			$this->db->where('receiving_id', $item->receiving_id);
			...
			$this->db->update('receivings_items', $data);
			$diff = $diff-$item->quantity_purchased;
		}
	}

	function have_quantity_confirmed($recv_id, $item_id, $quantity) {
		$this->db->where('receiving_id', $recv_id);
		$this->db->where('item_id', $item_id);
		$this->db->where('quantity_purchased', $quantity);
	
		$this->db->select('items_quantity_exp_confirmed, employee_confirmed_quantity_items_exp, date_items_quantity_exp_confirmed');
		$query = $this->db->get('receivings_items');
	
		if ($query->num_rows() > 0) {
			$row = $query->row();
	
			if ($row->items_quantity_exp_confirmed !== NULL && 
				$row->employee_confirmed_quantity_items_exp !== NULL && 
				$row->date_items_quantity_exp_confirmed !== NULL) {
				return true;
			}
		}
	...
	function get_all_items_expired_at_date($item_id, $expire_date) {
		$this->db->select('receiving_id, item_id, expire_date, quantity_purchased');
		$this->db->from('receivings_items');
		$this->db->where('item_id', $item_id);
		$this->db->where('expire_date', $expire_date);
		$this->db->where('items_quantity_exp_confirmed', NULL);
		$this->db->where('employee_confirmed_quantity_items_exp', NULL);
		$this->db->where('date_items_quantity_exp_confirmed', NULL);
	...
	function filter_item_expired_by_location($items_expired, $location) {
		$filtered_items = array();
	
		foreach ($items_expired as $item) {
			$this->db->select('receiving_id');
			$this->db->from('receivings');
			$this->db->where('receiving_id', (int)$item['receiving_id']);
	...
	function get_if_have_expired( $item_id ) {
