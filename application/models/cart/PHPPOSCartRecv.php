...
{		
	...
	public static function get_instance_from_recv_id($receiving_id,$cart_id = NULL)
	{
		...
		foreach($CI->Receiving->get_receiving_items($receiving_id)->result() as $row)
		{
			...
			$item_props['previous_cost'] = $cur_item_info->previous_cost;
	...
	public static function get_instance($cart_id)
	{
		...
		elseif ($has_return_mode) 
		{
			return new PHPPOSCartRecv(array('cart_id' => $cart_id, 'mode' => 'return'));
		}
		elseif ($has_purchase_order_mode) 
		{
			return new PHPPOSCartRecv(array('cart_id' => $cart_id, 'mode' => 'purchase_order'));
		}
		elseif ($has_transfer_mode) 
		{
			return new PHPPOSCartRecv(array('cart_id' => $cart_id, 'mode' => 'transfer'));
		}
		elseif ($has_store_account_payment_mode) 
		{
			return new PHPPOSCartRecv(array('cart_id' => $cart_id, 'mode' => 'store_account_payment'));
		}
		else
		{
			redirect('no_permitions_no_access/receivings');
		}
	...
	function get_total()
	{
		...
		{
			if($item->get_subtotal_receivings() >= 0)
			{
				$total+=$item->get_subtotal_receivings();	
			} else
			{
	...
	public function get_subtotal()
	{
		...
		foreach($this->get_items() as $item)
		{
			if($item->get_subtotal_receivings() >= 0)
			{
				$subtotal+=$item->get_subtotal_receivings();	
			} else
			{
