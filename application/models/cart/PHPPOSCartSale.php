...
{		
	...
	public static function get_instance_from_sale_id($sale_id,$cart_id = NULL,$is_editing_previous = FALSE,$copy_price_rules = TRUE)
	{
		...
		$cart->override_tax_class = !$CI->cart->is_tax_overrided() ? $sale_info['override_tax_class_id'] : NULL;
		...
		foreach($CI->Sale->get_sale_items($sale_id)->result() as $row)
		{
			...
			foreach($modifiers as $modifier)
			{
				...
				$item_props['modifier_items'][$modifier['modifier_item_id']] = array('id' =>$modifier['modifier_item_id'], 'name'=>$modifier_item_info['modifier_name'], 'display_name' => $display_name, 'unit_price' => $modifier['unit_price'],'cost_price' => $modifier['cost_price']);
			}
			...
			if ($item_props['tax_included'] && $cart->is_editing_previous && $item_props['taxable'])
			{
				...
				{
					$item_props['unit_price'] = to_currency_no_money(get_price_for_item_including_taxes($row->line, $item_props['unit_price'], $sale_id));
				}
			}
		...
		foreach($CI->Sale->get_sale_item_kits($sale_id)->result() as $row)
		{
			...
			foreach($modifiers as $modifier)
			{
				...
				$item_kit_props['modifier_items'][$modifier['modifier_item_id']] = array('id' =>$modifier['modifier_item_id'], 'name'=>$modifier_item_info['modifier_name'], 'display_name' => $display_name, 'unit_price' => $modifier['unit_price'],'cost_price' => $modifier['cost_price']);
			}
