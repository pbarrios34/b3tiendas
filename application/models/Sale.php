...
class Sale extends MY_Model
{
	...
	public function get_info_for_felplex($sale_id)
	{
		$this->db->from('sales');
		$this->db->where('sale_id',$sale_id);
	...
	function get_cf_customer_default_sale()
	{	
		$cf_array = array('CF','CONSUMIDOR FINAL','CLIENTE CF');
		$separetor_cf = implode(',',$cf_array);

		$this->db->select('p.person_id, p.full_name', false);
		$this->db->from('people as p');
		$this->db->join('customers as c','p.person_id = c.person_id', 'LEFT');
		$this->db->where_in('p.full_name', $cf_array);
	...
	{	
		...
		$sales_data = array(
			...
			'override_tax_class_id' => $cart->override_tax_class ? $cart->override_tax_class : NULL,
 		);
		...
		{			
			...
			if (property_exists($item,'item_id'))
			{
				...
				(
					...
					'is_repair_item' => $item->is_repair_item ? 1 : 0,
				);
	...
	function get_register_cash_added_subtracted_by_id($correlative)
	{
		$this->db->select('rla.id as correlative, rla.date, l.name as location_name, l.location_id,
		p.full_name, r.name as register_name, rl.register_id, rla.amount, rla.note, rla.payment_type');
		$this->db->from('register_log_audit as rla');
		$this->db->join('register_log as rl', 'rl.register_log_id = rla.register_log_id', 'inner');
		$this->db->join('registers as r', 'r.register_id = rl.register_id', 'inner');
		$this->db->join('employees as e', 'e.person_id = rla.employee_id', 'inner');
		$this->db->join('people as p', 'p.person_id = e.person_id', 'inner');
		$this->db->join('locations as l', 'l.location_id = r.location_id', 'inner');

		$this->db->where('rla.id=', $correlative);

		$registers_cash = $this->db->get()->result_array();

		return $registers_cash;
		
	}

	function get_all_register_cash_added_subtracted($mode)
	{
		if ($mode == 'add')
		{
			$this->db->select('rla.id as correlative, rla.date, l.name as location_name, 
			p.full_name, r.name as register_name, rla.amount, rla.note, rla.payment_type');
			$this->db->from('register_log_audit as rla');
			$this->db->join('register_log as rl', 'rl.register_log_id = rla.register_log_id', 'inner');
			$this->db->join('registers as r', 'r.register_id = rl.register_id', 'inner');
			$this->db->join('employees as e', 'e.person_id = rla.employee_id', 'inner');
			...
			$this->db->where('rla.amount >', 0);
			$this->db->order_by('rla.date');

			$registers_cash_added = $this->db->get()->result_array();

			return $registers_cash_added;
		}
		else
		{
			$this->db->select('rla.id as correlative, rla.date, l.name as location_name, 
			...
			$registers_cash_subtracted = $this->db->get()->result_array();

			return $registers_cash_subtracted;
		}
	}

	function get_register_cash_displayable_columns()
	{
		$return  = array(
			'correlative' => array('sort_column' => 'correlative', 'label' => lang('common_correlative')),
			'date' => array('sort_column' => 'date', 'label' => lang('common_date')),
			'location_name' => array('sort_column' => 'location_name', 'label' => lang('module_location')),
			'full_name' => array('sort_column' => 'full_name', 'label' => lang('sales_cash_employee')),
			'register_name' => array('html' => TRUE,'sort_column' => 'register_name', 'label' => lang('sales_cash_name')),
			'amount' => array('html' => TRUE,'sort_column' => 'amount', 'label' => lang('common_quantity'), 'format_function' => 'to_currency'),
			'note' => array('sort_column' => 'note', 'label' => lang('common_comments')),
			'payment_type' => array('sort_column' => 'payment_type', 'label' => lang('sales_transaction_type'))
		);	
	...
	function get_register_cash_default_columns()
	{
		return array('correlative','date','location_name','full_name','register_name','amount','note','payment_type');
	}
	...
	function get_sale_item($sale_id,$item_id,$line=false)
	{
		$variation_id = NULL;
	...
	function add_sale_item($sale_id,$item_id,$quantity=1,$serialnumber=null,$is_repair_item=false){
		...
		if($item_id == $this->Item->get_item_id(lang('work_orders_repair_item'))){
	...
	function sale_item_approved_by_update($sale_id,$item_id,$approved_by){
		$sale_item = $this->get_sale_item($sale_id,$item_id);
		if($sale_item){
			$variation_id = NULL;
	...
	function sale_item_assigned_to_update($sale_id,$item_id,$assigned_to){
		$sale_item = $this->get_sale_item($sale_id,$item_id);
		if($sale_item){
			$variation_id = NULL;
