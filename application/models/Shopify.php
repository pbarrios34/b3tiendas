...
class Shopify extends Ecom
{
	...
	public function cancel_subscription()
	{
		...
		$response = $this->do_delete("/admin/api/2022-07/recurring_application_charges/$charge_id.json");
		return TRUE;
	}
	function create_subscription()
	{
		...
		$charge_response = $this->do_post('/admin/api/2022-07/recurring_application_charges.json',array('recurring_application_charge' =>
		array(
	...
	public function update_item_from_phppos_to_ecommerce($item_id, $data = array())
	{
		...
		if ($data['manage_stock'])
		{
			...
			$this->do_post('/admin/api/2022-07/inventory_levels/set.json',$inv_data);			
		}		
	}
	...
	private function save_item($item)
	{
		...
		{
			$result = $this->do_put("/admin/api/2022-07/products/$ecom_prod_id.json",$data);
		}
		else
		{
			$result = $this->do_post('/admin/api/2022-07/products.json',$data);
		}
		...
		{					
			...
			if (count($result['product']['variants']) > 1 || $result['product']['variants'][0]['title'] != 'Default Title')
			{
				...
				foreach($phppos_item_variations as $phppos_variation_id => $phppos_item_variation)
				{
					...
					if ($this->do_post('/admin/api/2022-07/inventory_levels/set.json',$inv_data) === FALSE)
					{
						continue;
					}
					...
					if ($this->do_put("/admin/api/2022-07/inventory_items/$ecommerce_variation_inventory_item_id.json",$inventory_item_data) === FALSE)
					{
						continue;
					}
					...
					foreach($this->get_item_variation_images_for_ecommerce($phppos_variation_id) as $item_image)
					{
						...
						if ($this->do_put("/admin/api/2022-07/variants/$varient_id.json",$var_image_data) === FALSE)
						{
							continue;
						}
					}
			...
			else
			{
				...
				$this->do_put("/admin/api/2022-07/inventory_items/$ecommerce_inventory_item_id.json",$inventory_item_data);
	...
	{	
		...
		$response = $this->do_get('/admin/api/2022-07/products.json');
	...
	public function save_category($category_id)
	{
		...
		$response = $this->do_post('/admin/api/2022-07/smart_collections.json',$smart_collection);
	...
	public function update_category($category_id)
	{
		...
		$this->do_put('/admin/api/2022-07/smart_collections/'.$category->ecommerce_category_id.'.json',$smart_collection);
	}
	...
	public function delete_category($category_id)
	{
		...
		$this->do_delete('/admin/api/2022-07/smart_collections/'.$category->ecommerce_category_id.'.json');
	}
	...
	private function process_import_ecommerce_items_into_phppos($response)
	{
		...
		foreach($products as $product)
		{
			...
			if($ecommerce_last_modified > $item_last_modified)
			{
				...
				$inventory_item_response = $this->do_get("/admin/api/2022-07/inventory_items/$inventory_id.json");
	...
	public function delete_item($item_id)
	{
		...
		$this->do_delete("/admin/api/2022-07/products/$shopify_product_id.json");
	}
	...
	{		
		...
		{
			$response = $this->do_get('/admin/api/2022-07/orders.json?status=closed');			
		}
		else
		{
			$response = $this->do_get('/admin/api/2022-07/orders.json?status=any');
		}
